...
* Return: 0 on success
*/
int change_directory(char **cmd_input,
		list_t *env_var, int cmd_numb_str)
{
...
else if (cmd_input[1][0] == '-' && cmd_input[1][1] == '\0')
{
...
x_status = cd_cmd_exec(env_var, curr_dir,
		new_dir, cmd_input[1], cmd_numb_str);
free(curr_dir);
free(new_dir);
release_ptr(cmd_input);
return (x_status);
...
/**
* cd_without_cmd - change directory to home whnen only cd is entered
...
* @curr_dir: the directory in use
* @new_dir: the directory to be switched to
* @input_cmd: command entered by the user
...
int cd_cmd_exec(list_t *env_var, char *curr_dir,
		char *new_dir, char *input_cmd, int cmd_numb_strn)
{
int index = 0;
