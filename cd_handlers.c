...
new_dir = _strcat(new_dir, "/");
new_dir = _strcat(new_dir, cmd_input[1]);
}
else
{
new_dir = _strdup(cmd_input[1]);
}
}
}
else
...
* @curr_dir: the currenct working directory of the user
* @new_dir: desired new path
* @input_cmd: user command to the shell environ
